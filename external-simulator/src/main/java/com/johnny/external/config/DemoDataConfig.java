 ...
 * 
 * As the module uses native JDBC, it's not possible to use the handy features
 * of Hibernate etc. to create and populate the database, so it's all done 'by
 * hand'. Luckily the DB is very simple. Also, as it's not read until requests
 * are received, the DB can be initialised at any point during startup.
 * 
 * @author johnny
 *
 */
...
public class DemoDataConfig {
    ...
    /**
     * taken from an example in "Pro Spring Boot" - not sure if it's any better
     * than @PostConstruct in this case though.
     * 
     * @return see Spring doc :)
     */
    @Bean
