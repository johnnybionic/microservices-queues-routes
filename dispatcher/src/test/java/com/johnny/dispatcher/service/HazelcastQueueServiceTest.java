...
@SpringBootTest
public class HazelcastQueueServiceTest {
	...
	@InjectMocks
	private HazelcastQueueService service;
	...
	@Test
	public void thatDocumentReturnedIfWaitingOnQueue() {
		when(hazelcastInstance.getQueue(QUEUE_NAME)).thenReturn(queue);
		when(queue.size()).thenReturn(1);
		Document document = new Document(ID, null, null);
		when(queue.poll()).thenReturn(document );
